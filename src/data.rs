    ...
    match item {
        syn::Item::Struct(item) => {
            ...
            match fields {
                syn::Fields::Named(fields_named) => {
                    let impl_content = fields_named.named.iter().map(|field| {
                        ...
                        quote! {
                            ...
                            pub fn #set_method_name(&mut self, #field_name: #field_type){
                ...
                syn::Fields::Unnamed(fields_unnamed) => Err(syn::Error::new_spanned(fields_unnamed, "#[derive(Data)] doesn't support tuple struct")),
                syn::Fields::Unit => Ok(quote! {}),
            }
        }
        _ => Err(syn::Error::new_spanned(item, "#[derive(Data)] just support struct")),
    }
}